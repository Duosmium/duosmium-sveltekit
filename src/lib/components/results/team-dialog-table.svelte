...
<Table.Root>
  ...
  <Table.Body>
    {#each tableData as t}
      <Table.RowWithoutHover class="{Number(t.place.replace(/st|nd|rd|th/, '')) <= t.medals ? `place-${Number(t.place.replace(/st|nd|rd|th/, ''))}` : ''}">
        <Table.Cell class="font-medium px-4 py-2">{t.name}</Table.Cell>
        <Table.Cell class="px-4 py-2 text-center">{t.points}</Table.Cell>
        <Table.Cell class="px-4 py-2 text-center">{t.place.replace('undefinedth', 'n/a')}</Table.Cell>
