<script lang="ts">
  ...
  export let tableData: { name: string; points: string; place: string; notes: string; medals: number }[];
...
<Dialog.Root>
  ...
  <Dialog.Content class="max-h-[90vh] overflow-y-auto xs:max-w-[80vw] md:max-w-[60vw] xl:max-w-[40vw]">
    <Dialog.Header>
    ...
    <Dialog.Footer class="text-sm sm:justify-center hover:underline">
      <a href="/results/schools/{formattedSchoolName
							.charAt(0)
							.toLowerCase()}#{formattedSchoolName}">View results for {formattedSchoolName} in other tournaments</a>
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>