...
export async function findLogoPath(duosmiumID: string, images: string[] | undefined) {
...
export async function createLogoPath(duosmiumID: string, images: string[] | undefined) {
	...
	images =
		images ??
		(await supabase.storage.from('images').list('logos', { limit: 1048576 })).data?.map(
			(img) => img.name
		);
	let selected: string;
	if (images == null) {
