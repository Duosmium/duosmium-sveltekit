...
<html lang="en">
	<head>
		...
		<title
			...
			{'(Div. ' + interpreter.tournament.division + ')'}</title
		>
		...
		<link
			href="https://fonts.googleapis.com/css2?family=Roboto+Mono&family=Roboto+Slab&family=Roboto:ital,wght@0,300;0,400;0,500;0,700;1,400&display=swap"
			rel="stylesheet"
		/>
	</head>
	<body>
		<h1>
			{interpreter.tournament.year}
			{tournamentTitle(interpreter.tournament)}
			{'(Div. ' + interpreter.tournament.division + ')'}
		</h1>
		...
		<table>
			...
			{#each interpreter.teams as team}
				<tr>
					<td>{team.number}</td>
					<td
						>{formatSchool(team)}{team.suffix ? ' ' + team.suffix : ''}
						<span style="font-size: 0.75rem">{team.city ? team.city + ', ' : ''}{team.state}</span
						></td
					>
					<td>{team.rank}</td>
					<td>{team.points}</td>
					{#each team.placings as placing}
						<td
							>{placing.unknown && !placing.pointsLimitedByMaximumPlace
								? '??'
								: placing.isolatedPoints}</td
						>
					{/each}
				</tr>
			{/each}
