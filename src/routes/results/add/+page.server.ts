...
async function handleUploadedFile(file: File) {
	...
	if (!fileName.match(/\d{4}-\d{2}-\d{2}_([\w-]+_)*(invitational|regional|states|nationals)_(no_builds_)?[abc]/)) {
		throw new Error("The file name " + fileName + " doesn't match the approved syntax!");
	}
	const collection = db.collection("results");
	const resultExists = await collection.countDocuments({
		...
		allFiles.forEach(file =>
			{
				if (file === null || typeof file === 'string') {
					...
					throw new Error("String uploaded instead of file!");
				}
